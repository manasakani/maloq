...
nccls_str=("mpi" "nccl")
reorder_methods=("X" "CUSTOM" "METIS")
            ...
            for overlap in "${overlaps[@]}"; do
                for nccl in "${nccls[@]}"; do
                    for rcut in "${rcuts[@]}"; do
                        out_folder="weak3D_${rcut}A_${nccls_str[$nccl]}_${overlaps_str[$overlap]}_${hidden_dimension}hidden_${reorder}order_${reorder_method}"

                        mkdir -p $out_folder
                        rank=$(( init_rank * tile * tile * tile ))
                        # Loop through each rank configuration
...
python train_scaling.py -f /capstor/scratch/cscs/amaeder/ -rcut $rcut -nccl $nccl -overlap $overlap -hidden_dim $hidden_dimension -num_epochs 120 -is_reorder $reorder -reorder_method $reorder_method -tile_x $tile -tile_y $tile -tile_z $tile > ${out_folder}/output_\${SLURM_PROCID}_\${SLURM_NTASKS}.txt'
EOL

        ...
        for overlap in "${overlaps[@]}"; do
            for nccl in "${nccls[@]}"; do
                for rcut in "${rcuts[@]}"; do
                    out_folder="strong_${rcut}A_${nccls_str[$nccl]}_${overlaps_str[$overlap]}_${hidden_dimension}hidden_${reorder}order_${reorder_method}"
...
python train_scaling.py -f /capstor/scratch/cscs/amaeder/ -rcut $rcut -nccl $nccl -overlap $overlap -hidden_dim $hidden_dimension -num_epochs 120 -is_reorder $reorder -reorder_method $reorder_method > ${out_folder}/output_\${SLURM_PROCID}_\${SLURM_NTASKS}.txt'
EOL

...
import SO2 as SO2
import so2_model_local as so2_model
import SO3 as SO3
...
def main(folder):
    ...
    data_folder = os.path.join(folder, 'datasets/GST_monoclinic/')
    ...
    val_data_folder = os.path.join(folder, 'datasets/GST_monoclinic/')
    ...
    # Parameters:
    restart_file = None
    save_file = 'model_GST_'+str(world_size)+'test_rcut='+str(rcut)
    train_or_test = 'train'                                          
    ...
    cell = [[25.85676,0.00000,0.00000], [-17.23784,  29.85682,   0.00000], [0.00000,   0.00000,  41.69124]]
    ...
    atom_orbitals = {'32':[0,0,1,1,2], '51':[0,0,1,1,2], '52':[0,0,1,1,2]}                                           # Orbital types of each atom in the structure
    numbers = a_HfO2.atomic_numbers                                                       # Atomic numbers of each atom in the structure
    no_parity = True                                                                      # No parity symmetry          
    orbital_types = [[0,0,1,1,2],[0,0,1,1,2],[0,0,1,1,2]]                                                     # basis rank of each atom in the structure 
    ...
    if partitioning == 'slice_center':
        ...
        total_length = 5
        num_slices = 3
        data_loader = data.batch_data_HfO2_cartesian(a_HfO2, start, total_length, num_slices, equivariant_blocks=equivariant_blocks, out_slices=out_slices, construct_kernel=construct_kernel, dtype=torch.float32, slice_direction=2)
        print("Data loader created")

        start = 25
        total_length = 2
        num_slices = 1
        validation_loader = data.batch_data_HfO2_cartesian(a_HfO2_val, start, total_length, num_slices, equivariant_blocks=equivariant_blocks, out_slices=out_slices, construct_kernel=construct_kernel, dtype=torch.float32, slice_direction=2)

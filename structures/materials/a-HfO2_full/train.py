...
def main(folder, rcut, nccl, overlap, hidden_dim, num_epochs, is_reorder):
    ...
    tag = "HfO2_" + str(is_reorder) + "reorder_" + str(overlap) + "overlap"         # String tag for the output files
    ...
    num_epochs = num_epochs# 50000                                                              # Number of epochs                                                
    ...
    partition = {}
    partition['train'] = env.Domain_Decomp(a_HfO2_train, device, overlap=overlap, use_nccl=nccl)
    partition['validate'] = env.Domain_Decomp(a_HfO2_val, device, overlap=overlap, use_nccl=nccl)
    dist.barrier()

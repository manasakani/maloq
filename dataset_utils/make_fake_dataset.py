...
for folder_idx, structure_folder in enumerate(structure_folders[folder_start_idx:folder_end_idx]):
    ...
    # indices_to_delete = (   list(range(24, 31))         # oxygen s orbitals
    #                     )
    # fock_matrix = utils_orca_out.delete_rows_and_columns(fock_matrix, indices_to_delete)
    # basis = {8: [0, 0, 0, 0, 0, 1, 1, 1, 2, 2, 0, 1, 2], 1: [0, 0, 0, 1, 1]}
    # full_orb_list = np.hstack([basis[elements[i]] for i in range(len(elements))])
    ...
    # --> delete the d orbital (l = 3) for omol dzvp:
    # full_orb_list = np.hstack([basis[elements[i]] for i in range(len(elements))])
    # orbital_starts = np.hstack([0, np.cumsum([2*l + 1 for l in full_orb_list])[:-1]])
    # indices_to_delete = (list(range(9, 14)))   
    # fock_matrix = utils_orca_out.delete_rows_and_columns(fock_matrix, indices_to_delete)
    # basis = {8: [0, 0, 0, 1, 1], 1: [0, 0, 1]}
    # --------------------------------------------------------------------------------------

    fock_matrix = utils_orca_out.sort_by_m_QM7(fock_matrix, basis, np.array(elements))

    # if folder_idx == 1:
    #     plt.imshow(np.log(np.abs(fock_matrix)))
    #     plt.colorbar()
    #     plt.savefig("omol_fock.png", dpi=300, bbox_inches='tight')
    #     exit()

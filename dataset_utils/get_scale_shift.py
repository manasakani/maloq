...
def get_scale_shift(database, dataset_name, rcut, dtype=torch.float32, reduce_edge=False):
    """
    Compute scaling and shifting factors for the scalar components of the hamiltonian datasets and save them to file
    """

    num_molecules = 10000 #len(database)
    ...
    for Z, tensor_list in element_scalar_values.items():
        ...
        stds = stacked.std(dim=0, unbiased=False)  # unbiased =False for population std
    ...
    torch.save(scale_shift_data, "./fock_datasets/element_scale_shifts_uracil_" + dataset_name + ".pt")
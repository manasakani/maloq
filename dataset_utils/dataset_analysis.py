...
def dataset_analysis(database, dataset_name, rcut=5.0, dtype=torch.float64, reduce_edge=False, scale_shift_data=None):
    """
    Compute scaling and shifting factors for the scalar components of the hamiltonian datasets and save them to file
    """

    num_molecules = 1000 #len(database)
    ...
    for i in range(num_molecules):
        ...
        for atomic_number, node_block in zip(atomic_numbers, graph_targets.node_labels):
            ...
            nonzero_values = node_block[np.abs(node_block) >= tol]  # keep only
    ...
    np.save(f"{dataset_name}_element_node_block_values_scaled.npy", element_node_block_values)
    ...
    index_track = 0
    element_labels = []
    for atomic_number, node_blocks in element_node_block_values.items():
    ...
    plt.savefig(f"{dataset_name}_node_block_values_scaled.png", bbox_inches='tight', dpi=300)
    ...
    plt.savefig(f"{dataset_name}_node_block_values_violin_scaled.png", bbox_inches='tight', dpi=300)

...
The output contains `energy`, `forces`, `hamiltonian`, and `overlap` properties.
Raw QH9Stable does not include forces or energy, so both are explicit zero
placeholders. These semantics are recorded in database
metadata; this database is intended for Hamiltonian training, not energy or
force training.

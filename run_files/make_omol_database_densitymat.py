...
def parse_args():
    ...
    parser.add_argument('--matrix_type', type=str, default='fock',
                       help='Type of matrix to process (default: fock)')
...
def main():
    ...
    # orca file is in a "step_X" subfolder for each structure, where X can vary:
    orca_file = 'orca.out'
    ...
    for folder_idx, structure_folder in enumerate(structures_to_process):
        ...
        if open_shell:
            ...
            fock_matrices, elements, coordinates, _ = utils_orca_out.read_orca_out(orca_output_filepath, unrestricted=open_shell, get_fock=get_fock)
            ...
            fock_matrix, elements, coordinates, _ = utils_orca_out.read_orca_out(orca_output_filepath, get_fock=get_fock)
            if check_elements(elements, full_basis) == False:
                print(f"Skipping {structure_folder}: {elements} not in basis", flush=True)
                continue
            basis = {element: full_basis[element] for element in elements} # Get basis (for this structure) for rearranging the matrix:
        ...
        if not all(element in full_basis for element in elements):
            print(f"Skipping {structure_folder}: {elements} not in basis", flush=True)
            continue

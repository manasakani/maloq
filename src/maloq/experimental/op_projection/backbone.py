...
"""Node-latent MALOQ-NTE-V2 backbone for implicit operator projection.

This experimental variant keeps the selected NTE-V2 node trunk but deliberately
does not build or return a persistent learned edge representation. A later
operator-projection callback consumes the node latents together with the graph
geometry and streams pair contributions directly into a probe-vector result.

Edge geometry is still required. "Node-only" here means that the learned
backbone output contains only node features; it does not mean that interatomic
operator terms are omitted.
"""
...
from ...helm.nn.radial import GaussianSmearing, PolynomialEnvelope, RadialMLP
from ...helm.nn.so3_layers import SO3_Linear
from .block import NodeBlock


OP_PROJECTION_ARCHITECTURE = "MALOQ-NTE-V2-OP-PROJECTION"
...
@registry.register_model("op_projection_backbone")
class OpProjectionBackbone(nn.Module):
    """NTE-V2 node trunk that exposes geometry for a lazy operator callback."""

    architecture = OP_PROJECTION_ARCHITECTURE
    ...
    def __init__(
        ...
        *,
        ...
        # ``qhflow3_exact`` deliberately ignores ``base_scalar`` in
        # MatrixEmbedding.forward. Keep these modules for reference-state
        # compatibility, but do not expose permanently dormant parameters to
        # DDP or the optimizer.
        self.charge_embedding.requires_grad_(False)
        self.spin_embedding.requires_grad_(False)
        self.scalar_node_embedding.requires_grad_(False)
    ...
    def _run_message_passing(
        ...
        for block in self.node_blocks:
            # NodeBlock has no recurrent learned edge state. Pair geometry
            # enters through x_edge, edge_index, and the Wigner maps.
            node_state = block(node_state, None, *block_args)
        return node_state
    ...
    def forward(self, batch, batch_index=None, output_dir=None):
        ...
        if getattr(batch, "distributed_graph_training", False):
            raise ValueError("op_projection supports data-parallel training only.")
        ...
        node_state = self._run_message_passing(
            node_state,
            x_edge,
            graph,
            wigner,

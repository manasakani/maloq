...
class Domain_Decomp():
    def __init__(self, structure, device, partition_type='linear'):
        ...
        # 1. Determine max node index to size the array
        # We use a safe max, defaulting to 0 if there are no nodes
        all_nodes_flat = [node for nodes in self.all_local_node_indices for node in nodes]
        self.total_global_nodes = max(all_nodes_flat) + 1 if all_nodes_flat else 0
        
        # 2. Build the inverse mapping (node -> rank)
        self.node_to_rank_map = np.zeros(self.total_global_nodes, dtype=np.int32)
        for rank, nodes in enumerate(self.all_local_node_indices):
            if len(nodes) > 0:
                self.node_to_rank_map[nodes] = rank
        # ----------------------
    ...
    def init_comm_pattern_expand(self, src0_or_dst1):
        ...
        with torch.no_grad():
            ...
            raw_edges = self.local_edges[src0_or_dst1, :]
            if hasattr(raw_edges, 'detach'):
                edge_index = raw_edges.detach().cpu().numpy()
            else:
                edge_index = np.ascontiguousarray(raw_edges)
            
            # 1. Rank Lookup
            node_ranks = self.node_to_rank_map[edge_index]
            is_remote_mask = (node_ranks != self.rank)
            
            remote_nodes = edge_index[is_remote_mask]
            remote_node_ranks = node_ranks[is_remote_mask]

            # 2. Recv Logic: Strictly preserved insertion order
            nodes_to_recv = {}
            for i, remote_rank in enumerate(remote_node_ranks):
                rank_int = int(remote_rank)
                if rank_int not in nodes_to_recv:
                    nodes_to_recv[rank_int] = []
                
                node_val = int(remote_nodes[i])
                if node_val not in nodes_to_recv[rank_int]:
                    nodes_to_recv[rank_int].append(node_val)

            # 3. Send Logic: Strictly preserved order (Looping 0 to Size)
            global_edge_nodes = self.global_edge_index[src0_or_dst1, :]
            nodes_to_send = {}

            # Convert local nodes to a boolean mask for fast lookup
            is_local_node_mask = np.zeros(self.total_global_nodes, dtype=bool)
            is_local_node_mask[self.local_node_indices] = True

            for i in range(self.size):
                if i == self.rank: continue
                
                # Identify the nodes associated with the target rank's edge list
                # We get the nodes involved in the target rank's edges
                target_rank_nodes = global_edge_nodes[self.all_local_edge_indices[i]]
                
                # Find which of those nodes are local to ME (rank)
                # This is the boolean mask equivalent of the intersection
                send_mask = is_local_node_mask[target_rank_nodes]
                
                # Get the unique, sorted nodes to send
                to_send = np.unique(target_rank_nodes[send_mask])
                
                # Remove any nodes that are strictly local to the target rank 
                if len(to_send) > 0:
                    nodes_to_send[i] = to_send.tolist()

            # 4. Vectorized Indices to Send
            node_to_local_pos = torch.full((self.total_global_nodes,), -1, dtype=torch.long)
            node_to_local_pos[torch.from_numpy(self.local_node_indices)] = torch.arange(len(self.local_node_indices))
            
            indices_to_send = {}
            for target_rank, nodes in nodes_to_send.items():
                nodes_t = torch.tensor(nodes, dtype=torch.long)
                indices_to_send[target_rank] = node_to_local_pos[nodes_t].to(self.device)

            # 5. Local/Remote Indices
            is_local_mask = ~is_remote_mask
            local_indices = [int(i) for i in node_to_local_pos[torch.from_numpy(edge_index[is_local_mask])]]
            
            # logic for remote indices to ensure parity
            remote_indices = torch.ones(len(remote_nodes), dtype=torch.long, device=self.device) 
            node_track = 0
            for rank, nodes in nodes_to_recv.items():
                for node in nodes:
                    mask = (remote_nodes == node)
                    remote_indices[torch.from_numpy(mask).to(self.device)] = node_track
                    node_track += 1

            return {
                'local_indices': local_indices,
                'remote_indices': remote_indices,
                'is_local': is_local_mask,
                'is_remote': is_remote_mask,
                'nodes_to_send': nodes_to_send,
                'indices_to_send': indices_to_send,
                'nodes_to_recv': nodes_to_recv,
                'indices_to_send_cp': {k: cp.asarray(v) for k, v in indices_to_send.items()},
                'local_indices_torch': torch.tensor(local_indices, dtype=torch.long, device=self.device),
                'remote_indices_torch': remote_indices
            }

    # def init_comm_pattern_expand_ref(self, src0_or_dst1):

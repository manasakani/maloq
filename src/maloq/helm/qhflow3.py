"""QHFlow3 feature backbone.

This module contains the maintained QHFlow3 density feature path. It preserves
the tensor-expansion feature contract while omitting the historical
QHFlow2/QHFlow3 inheritance stack and inactive SelfNet/PairNet branches.

The lower-level eSCN math modules remain local so the active path can be
inspected without jumping through the old monolithic backbone class.
"""
...
class GridAtomwise(torch.nn.Module):
    ...
    The default QHFlow3 path uses this grid variant.
    """
...
class eSCNMD_Block(torch.nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        if ff_type != "grid":
            raise ValueError("QHFlow3 keeps only the grid atomwise eSCN path.")
        self.sphere_channels = sphere_channels
        self.hidden_channels = hidden_channels
        self.lmax = lmax
        self.mmax = mmax
...
# QHFlow3Features no longer imports the vendor backbone class itself.
class QHFlow3ESCNBackboneHam(nn.Module):
    def __init__(
        ...
        # rotation utils
        Jd_list = torch.load(_QHFLOW3_JD_PATH)
    ...
    def _run_xy_stack(
        ...
        if xy_pair_message is None:
            raise ValueError("QHFlow3 requires at least one xy_block.")
    ...
    def forward(self, data_dict: Any, ham_features: list[torch.Tensor]) -> dict[str, torch.Tensor]:
        ...
        if "edge_index" not in data_dict:
            raise ValueError("QHFlow3 expects precomputed edge_index.")
        graph_dict = {
        ...
        if n_pair_edges == 0:
            raise ValueError("QHFlow3 requires at least one pair edge.")
...
class QHFlow3Features(nn.Module):
...
class QHFlow3Backbone(QHFlow3Features):

...
"""Fixed message-passing blocks used by :mod:`esen_osh_v2`.

There are no legacy mode selectors in this module.  Each public block class
implements one equation and is named by its role in the fixed architecture.
"""
...
class DegreeScale(nn.Module):
    ...
    def degree_scales(self):
        return self.init * torch.exp(
            self.log_range * torch.tanh(self.raw)
        )
...
class InitialEdgeBlock(_Block):
    """Initialize E from EdgeWise(N), then add scaled AtomWise(E)."""
...
class EdgeRefinementBlock(_Block):
    """Refine E with AtomWise-scaled EdgeWise messages from N."""
    ...
    def forward(
        ...
        return edge_state + self._atomwise(
            self.edge_update_scale(local)
        )

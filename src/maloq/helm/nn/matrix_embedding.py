"""Matrix-to-scalar embeddings for atom-wise equivariant models."""
...
class MatrixEmbedding(nn.Module):
    """Embed atom-local matrix blocks as invariant scalar features.

    ``overlap`` augments an existing atom-wise scalar with overlap features.
    ``qhflow3_exact`` reproduces the active QHFlow3 zero-H/S/time/atom mixing
    path used by the fixed V2 architecture.
    """
    ...
    def __init__(
        ...
        *,
        ...
        if mode not in self.MODES:
            raise ValueError(f"Unsupported matrix embedding mode: {mode!r}.")
        if delta_target not in {"fock_matrix", "density_matrix"}:
            raise ValueError("delta_target must be 'fock_matrix' or 'density_matrix'.")
    ...
    def _matrix_blocks(
        ...
        if matrices is None:
            raise ValueError(
                f"Matrix embedding mode {self.mode!r} requires batch.{attribute}."
            )
        if not isinstance(matrices, (list, tuple)):
            matrices = [matrices]
        if not hasattr(batch, "ptr"):
            raise ValueError("Matrix embedding requires PyG batch pointers.")

        ptr = batch.ptr.detach().cpu().tolist()
        if len(matrices) != len(ptr) - 1:
            raise ValueError(
                f"Expected {len(ptr) - 1} {matrix_label} matrices, got {len(matrices)}."
            )
        ...
        for graph_index, matrix in enumerate(matrices):
            ...
            for atomic_number in atoms_cpu:
                if atomic_number not in self._orbital_masks:
                    raise ValueError(
                        f"Matrix embedding basis {self.basis!r} does not support "
                        f"Z={atomic_number}."
                    )
    ...
    def system_embedding(
        ...
        *,
        ...
        """Return the fixed zero-charge/spin feature used for block injection."""
        if self.mode != "qhflow3_exact":
            raise ValueError(

...
from dataset_utils.ASEDataset import ASEAtomsData, ASEDataset, distribute_data
...
# ---------------------------
# --> OMOL
dataset_folder = '/capstor/store/cscs/pasc/c33/manasa/omol_datasets/omol_electrolytes_unsolvated/'
total_rows = 28800
output_folder = 'outputs_omol_electrolytes'
dataset_name = 'omol'
...
# --> Training settings:
train_or_eval = "train"
num_val = world_size                             # Number of validation structures
...
target_edges_per_batch = 8000          # Don't use more than 18k (for E128)
...
# Distribute Data using the fixed global counts
num_db_files = len([f for f in os.listdir(dataset_folder) if f.endswith('.db')])
train_data_dict, val_data_dict = distribute_data(
    base_folder=dataset_folder,
    N_db_files=num_db_files,
    world_size=world_size,
    rank=rank,
    N_global_train=num_train,
    N_global_val=num_val
)
train_local_num_mol = sum(e['end_idx'] - e['start_idx'] for e in train_data_dict)
val_local_num_mol = sum(e['end_idx'] - e['start_idx'] for e in val_data_dict)

train_databases = []

if rank == 0:
    print(f"Rank {rank}: Loading {len(train_data_dict)} training segments.", flush=True)

# Iterate through every segment assigned to this rank (potentially from multiple files)
for entry in train_data_dict:
    # entry is a dictionary: {'db_file': str, 'start_idx': int, 'end_idx': int}
    
    # Instantiate ASEDataset for the specific file and local index range
    dataset = ASEDataset(
        db_path=entry['db_file'], # Use the specific database file path
    ...
    train_databases.append(dataset)

if not train_databases:
    # This should only happen if N_global_train was 0 or the distribution was highly skewed/bugged
    raise RuntimeError(f"Rank {rank} was not assigned any training data segments.")

# Combine all dataset segments into one dataset for the DataLoader
train_database = ConcatDataset(train_databases)
# -------------------------------------------------------------

# --- 2. Load and Concatenate Validation Data for this Rank ---
val_databases = []

if rank == 0:
    print(f"Rank {rank}: Loading {len(val_data_dict)} validation segments.", flush=True)
...
for entry in val_data_dict:
    ...
    val_databases.append(dataset)

if not val_databases:
    # Handle the case where N_global_val might be 0, or just highly skewed
    val_database = None # Or an empty list/dataset, depending on your DataLoader's requirements
else:
    val_database = ConcatDataset(val_databases)
# -------------------------------------------------------------

# 3. Update local molecule counts (Optional, but recommended for logging)
num_train = len(train_database)
num_val = len(val_database) if val_database else 0

print(f"Total training molecules loaded by rank {rank}: {num_train}", flush=True)
print(f"Total validation molecules loaded by rank {rank}: {num_val}", flush=True)
...
if scale_and_shift:
    ...
    if scale_shift_file not in os.listdir('./fock_utils/'):
        ...
        get_scale_shift.get_scale_shift(train_database, dataset_name, rcut_orbitals, dtype=dtype, reduce_edge=reduce_edge, filename=scale_shift_file, open_shell=open_shell)
...
if train_or_eval == "train":
    train_data = get_scale_shift.scale_shift_database(train_database, 0, train_local_num_mol, rcut_orbitals, orbital_basis, reduce_edge, scale_shift_data, dataset_name, scale_nodes=scale_and_shift, train_or_eval=train_or_eval, open_shell=open_shell)
    ...
    val_data = get_scale_shift.scale_shift_database(val_database, 0, val_local_num_mol, rcut_orbitals, orbital_basis, reduce_edge, scale_shift_data, scale_nodes=scale_and_shift, train_or_eval=train_or_eval, open_shell=open_shell)

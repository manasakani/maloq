...
_multiple_matrix2label = cp.RawKernel(r'''
#define MAX_ELEMENTS 100

extern "C" __global__
void _multiple_matrix2label(
...
def cupy_multiple_matrix2label(
    ...
    # assert dtype of fock and targets is doubld
    for t in targets:
        assert t.dtype == cp.float64
    for f in fock_matrices:
        assert f.dtype == cp.float64
    ...
    fock_matrices_strides_m = cp.array(
        [fock_matrix.strides[0] // double_size for fock_matrix in fock_matrices],
        dtype=cp.int32
    )
    fock_matrices_strides_n = cp.array(
        [fock_matrix.strides[1] // double_size for fock_matrix in fock_matrices],
        dtype=cp.int32
    )
    ...
    target_strides_m = cp.array(
        [t.strides[0] // double_size for t in targets],
        dtype=cp.int32
    )
    target_strides_n = cp.array(
        [t.strides[1] // double_size for t in targets],
        dtype=cp.int32
    )
    ...
    _multiple_matrix2label(
        ...
        shared_mem = double_size * max_block_size**2,
    )

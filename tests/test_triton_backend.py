...
import fock_utils.triton_backend as foo2_old
import fock_utils.triton_backend as foo2
import fock_utils.triton_backend as foo2_l2
...
def run_benchmark_for_basis(basis_name, orbital_basis, device, dtype, num_edges=100000, n_iters=100, n_warmup=10):
    ...
    decomp_triton_old = foo2_old.TritonE3TensorDecomp(decomp)
    ...
    if foo2_l2 is not None and hasattr(foo2_l2, "TritonE3TensorDecompL2"):
        decomp_triton_l2 = foo2_l2.TritonE3TensorDecompL2(decomp)
    ...
    # 6.5 BENCHMARK: get_H (TRITON OLD)
    print(f"\n[Benchmarking get_H (Triton OLD) over {n_iters} iterations...]")
    for _ in range(n_warmup): # warmup triton specific JIT compiling
        _ = decomp_triton_old.get_H(dummy_net_out)
    torch.cuda.synchronize()

    times = []
    for _ in range(n_iters):
        start = time.perf_counter()
        out_h_triton_old = decomp_triton_old.get_H(dummy_net_out)
        torch.cuda.synchronize()
        end = time.perf_counter()
        times.append((end - start) * 1000)
    import statistics
    get_H_triton_old_time_ms = statistics.median(times)
    print(f"--> get_H (Triton OLD) median time: {get_H_triton_old_time_ms:.3f} ms")
    print(f"    Speedup over get_H_original: {get_H_original_time_ms / get_H_triton_old_time_ms:.2f}x")

    match_triton_old_ref = torch.allclose(out_h, out_h_triton_old, atol=1e-5, rtol=1e-5)
    rel_error_old = torch.norm(out_h - out_h_triton_old) / torch.norm(out_h)

    # 6.6 BENCHMARK: get_H (TRITON NEW)
    print(f"\n[Benchmarking get_H (Triton NEW) over {n_iters} iterations...]")
    for _ in range(n_warmup): # warmup triton specific JIT compiling
    ...
    impl_results = [
        ...
        {
            "basis": basis_name,
            "name": "Triton naive (OLD)",
            "time_ms": get_H_triton_old_time_ms,
            "speedup_vs_original": get_H_original_time_ms / get_H_triton_old_time_ms,
            "match": bool(match_triton_old_ref),
            "rel_error": rel_error_old.item(),
            "tflops": useful_flop / (get_H_triton_old_time_ms / 1000.0) / 1e12,
        },
        {
            "basis": basis_name,
            "name": "Triton naive (NEW)",

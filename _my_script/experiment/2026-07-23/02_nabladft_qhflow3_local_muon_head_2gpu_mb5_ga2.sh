...
# QHFlow3-clean + MUON-compatible MALOQ head on the same rcut=8 local matrix
# objective as the MALOQ baselines.
...
CONFIG=${PROJECT_ROOT}/_my_script/experiment/2026-07-23/qhflow3_local_muon_head_nabladft.yaml
...
GROUP_ROOT=${PROJECT_ROOT}/outputs/nabladft-qhflow3-local-muon-head-2gpu-eb20-mb5-ga2-${SCOPE_SLUG}-seed44-${RUN_ID}
...
printf 'backbone\thead_type\trcut_orbitals\tgpus\tmicro_batch\tworld_size\taccumulation\teffective_batch\tconfig\n' \
  > "${GROUP_ROOT}/launch_manifest.tsv"
printf 'qhflow3_clean\tmaloq_muon\t8.0\t%s\t5\t2\t2\t20\t%s\n' "${GPUS}" "${CONFIG}" \
  >> "${GROUP_ROOT}/launch_manifest.tsv"
...
echo "QHFlow3-local ${SCOPE}: ${status} (exit ${exit_code})"

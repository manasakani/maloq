...
case "${ABLATION}" in
  node)
    CONFIG=${SCRIPT_ROOT}/nte64e2_qcond_qhflow3_exact_node_nabladft.yaml
    SLUG=qhfnodeexact-v2
    DEFAULT_PORT=29661
    WANDB_DISPLAY_NAME="NablaDFT | NTE-64/2 | MatrixMuon+AuxAdamW | RAW | QHFcond | QHFNodeExact | V2"
    ;;
  pair)
    CONFIG=${SCRIPT_ROOT}/nte64e2_qcond_qhflow3_exact_pair_nabladft.yaml
    SLUG=qhfpairexact-v2
    DEFAULT_PORT=29662
    WANDB_DISPLAY_NAME="NablaDFT | NTE-64/2 | MatrixMuon+AuxAdamW | RAW | QHFcond | QHFPairExact | V2"
    ;;
  blocks)
    CONFIG=${SCRIPT_ROOT}/nte64e2_qcond_qhflow3_exact_blocks_nabladft.yaml
    SLUG=qhfblocksexact-v2
    DEFAULT_PORT=29663
    WANDB_DISPLAY_NAME="NablaDFT | NTE-64/2 | MatrixMuon+AuxAdamW | RAW | QHFcond | QHFBlocksExact | V2"
    ;;
  *)
    echo "Unknown ablation '${ABLATION}'. Use node, pair, or blocks." >&2
    exit 2
    ;;
esac
...
case "${SCOPE}" in
  ...
  full)
    RUN_ARGS=(
      ...
      --wandb-group nabla-nte64-qhflow3-layer-transplant-v1
      --wandb-run-name "${WANDB_DISPLAY_NAME}"
    )
    SCOPE_SLUG=full-e20
    ;;
  *)
    echo "Usage: $0 {validate|smoke|full} {node|pair|blocks} GPU0,GPU1" >&2
    exit 2
    ;;
esac
...
OUTPUT_ROOT=${PROJECT_ROOT}/outputs/nabladft-nte64-${SLUG}-2gpu-eb20-mb5-ga2-${SCOPE_SLUG}-${RUN_ID}
...
if [[ "${SCOPE}" == "smoke" && ${EXIT_CODE} -eq 0 ]]; then
  case "${OUTPUT_ROOT}" in
    "${PROJECT_ROOT}"/outputs/nabladft-nte64-qhf*exact-v2-2gpu-eb20-mb5-ga2-full-size-smoke-e1-*) ;;
    *)

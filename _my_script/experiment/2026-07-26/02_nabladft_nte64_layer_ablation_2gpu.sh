...
case "${ABLATION}" in
  node2)
    CONFIG=${SCRIPT_ROOT}/nte64e2_qcond_node2_nolayerscale_nabladft.yaml
    SLUG=node2-nols
    PORT=29641
    ;;
  qhfpair)
    CONFIG=${SCRIPT_ROOT}/nte64e2_qcond_qhflow3_parallel_edge_nabladft.yaml
    SLUG=qhf-parallel-edge
    PORT=29642
    ;;
  projadamw)
    CONFIG=${SCRIPT_ROOT}/nte64e2_qcond_projection_adamw_nabladft.yaml
    SLUG=projection-adamw
    PORT=29643
    ;;
  repeatsys)
    CONFIG=${SCRIPT_ROOT}/nte64e2_qcond_repeat_system_nabladft.yaml
    SLUG=repeat-system-node-block
    PORT=29644
    ;;
  nteparallel)
    CONFIG=${SCRIPT_ROOT}/nte64e2_qcond_nte_parallel_edge_nabladft.yaml
    SLUG=nte-parallel-edge
    PORT=29645
    ;;
  *)
    echo "Unknown ablation '${ABLATION}'. Use node2, qhfpair, projadamw, repeatsys, or nteparallel." >&2
    exit 2
    ;;
esac
...
case "${SCOPE}" in
  ...
  full)
    RUN_ARGS=(
      ...
      --wandb-group nabla-nte64-layer-ablation-v1
    )
    SCOPE_SLUG=full-e20
    ;;
  *)
    echo "Usage: $0 {validate|smoke|full} {node2|qhfpair|projadamw|repeatsys|nteparallel} GPU0,GPU1" >&2
    exit 2
    ;;
esac
...
if [[ "${SCOPE}" == "smoke" && ${EXIT_CODE} -eq 0 ]]; then
  case "${OUTPUT_ROOT}" in
    "${PROJECT_ROOT}"/outputs/nabladft-nte64-*-2gpu-eb20-mb5-ga2-full-size-smoke-e1-*) ;;
    *)

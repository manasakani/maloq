...
CONFIG=${SCRIPT_ROOT}/nte64e2_qcond_source_target_message_nabladft.yaml
...
PORT=29646
WANDB_DISPLAY_NAME="NablaDFT | NTE-64/2 | MatrixMuon+AuxAdamW | RAW | QHFcond | STMessage | V1"
...
OUTPUT_ROOT=${PROJECT_ROOT}/outputs/nabladft-nte64-stmessage-2gpu-eb20-mb5-ga2-${SCOPE_SLUG}-${RUN_ID}
...
if [[ "${SCOPE}" == "smoke" && ${EXIT_CODE} -eq 0 ]]; then
  case "${OUTPUT_ROOT}" in
    "${PROJECT_ROOT}"/outputs/nabladft-nte64-stmessage-2gpu-eb20-mb5-ga2-full-size-smoke-e1-*) ;;
    *)

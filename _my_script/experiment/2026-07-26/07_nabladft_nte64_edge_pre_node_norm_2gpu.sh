...
CONFIG=${SCRIPT_ROOT}/nte64e2_qcond_edge_pre_node_norm_nabladft.yaml
PORT=29654
WANDB_DISPLAY_NAME="NablaDFT | NTE-64/2 | MatrixMuon+AuxAdamW | RAW | QHFcond | EdgePreNodeNorm | V1"
...
OUTPUT_ROOT=${PROJECT_ROOT}/outputs/nabladft-nte64-edge-pre-node-norm-2gpu-eb20-mb5-ga2-${SCOPE_SLUG}-${RUN_ID}
...
if [[ "${SCOPE}" == "smoke" && ${EXIT_CODE} -eq 0 ]]; then
  case "${OUTPUT_ROOT}" in
    "${PROJECT_ROOT}"/outputs/nabladft-nte64-edge-pre-node-norm-2gpu-eb20-mb5-ga2-full-size-smoke-e1-*) ;;
    *)

...
case "${MODE}" in
  proj)
    CONFIG=${SCRIPT_ROOT}/nte64e2_qcond_qhflow3_irrep_projection_nabladft.yaml
    SLUG=qhfproj-v1
    WANDB_DISPLAY_NAME="NablaDFT | NTE-64/2 | MatrixMuon+AuxAdamW | RAW | QHFcond | QHFProj | V1"
    ;;
  edgepre-proj)
    CONFIG=${SCRIPT_ROOT}/nte64e2_qcond_edgepre_qhflow3_irrep_projection_nabladft.yaml
    SLUG=edgepre-qhfproj-v1
    WANDB_DISPLAY_NAME="NablaDFT | NTE-64/2 | MatrixMuon+AuxAdamW | RAW | QHFcond | EdgePre+QHFProj | V1"
    ;;
  *)
    echo "Unknown mode '${MODE}'. Use proj or edgepre-proj." >&2
    exit 2
    ;;
esac
...
case "${SCOPE}" in
  ...
  full)
    RUN_ARGS=(
      ...
      --wandb-group nabla-nte64-output-projection-ablation-v1
      --wandb-run-name "${WANDB_DISPLAY_NAME}"
    )
    SCOPE_SLUG=full-e20
    ;;
  *)
    echo "Usage: $0 {validate|smoke|full} {proj|edgepre-proj} GPU0,GPU1" >&2
    exit 2
    ;;
esac
...
if [[ "${SCOPE}" == "smoke" && ${EXIT_CODE} -eq 0 ]]; then
  case "${OUTPUT_ROOT}" in
    "${PROJECT_ROOT}"/outputs/nabladft-nte64-qhfproj-v1-2gpu-eb20-mb5-ga2-full-size-smoke-e1-* | \
    "${PROJECT_ROOT}"/outputs/nabladft-nte64-edgepre-qhfproj-v1-2gpu-eb20-mb5-ga2-full-size-smoke-e1-*) ;;
    *)

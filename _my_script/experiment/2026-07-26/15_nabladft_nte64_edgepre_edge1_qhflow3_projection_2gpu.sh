...
CONFIG=${SCRIPT_ROOT}/nte64e2_qcond_edgepre_edge1_qhflow3_irrep_projection_nabladft.yaml
WANDB_DISPLAY_NAME="NablaDFT | NTE-64/2 | MatrixMuon+AuxAdamW | RAW | QHFcond | EdgePre+Edge1+QHFProj | V1"
...
case "${SCOPE}" in
  ...
  full)
    RUN_ARGS=(
      ...
      --wandb-group nabla-nte64-final-mae-composition-v1
      --wandb-run-name "${WANDB_DISPLAY_NAME}"
    )
    SCOPE_SLUG=full-e20
    ;;
...
OUTPUT_ROOT=${PROJECT_ROOT}/outputs/nabladft-nte64-edgepre-edge1-qhfproj-v1-2gpu-eb20-mb5-ga2-${SCOPE_SLUG}-${RUN_ID}
...
if [[ "${SCOPE}" == "smoke" && ${EXIT_CODE} -eq 0 ]]; then
  case "${OUTPUT_ROOT}" in
    "${PROJECT_ROOT}"/outputs/nabladft-nte64-edgepre-edge1-qhfproj-v1-2gpu-eb20-mb5-ga2-full-size-smoke-e1-*) ;;
    *)

...
case "${ABLATION}" in
  postrms)
    CONFIG=${SCRIPT_ROOT}/nte64e2_qcond_edge_postrms_nabladft.yaml
    ABLATION_SLUG=edge-postrms
    PORT=29650
    WANDB_DISPLAY_NAME="NablaDFT | NTE-64/2 | MatrixMuon+AuxAdamW | RAW | QHFcond | EdgePostRMS | V1"
    ;;
  split)
    CONFIG=${SCRIPT_ROOT}/nte64e2_qcond_edge_splitnorm_nabladft.yaml
    ABLATION_SLUG=edge-splitnorm
    PORT=29651
    WANDB_DISPLAY_NAME="NablaDFT | NTE-64/2 | MatrixMuon+AuxAdamW | RAW | QHFcond | EdgeSplitNorm | V1"
    ;;
  degree)
    CONFIG=${SCRIPT_ROOT}/nte64e2_qcond_edge_degreenorm_nabladft.yaml
    ABLATION_SLUG=edge-degreenorm
    PORT=29652
    WANDB_DISPLAY_NAME="NablaDFT | NTE-64/2 | MatrixMuon+AuxAdamW | RAW | QHFcond | EdgeDegreeNorm | V1"
    ;;
  *)
    echo "Unknown ablation: ${ABLATION}. Use postrms, split, or degree." >&2
    exit 2
    ;;
esac
...
case "${SCOPE}" in
  ...
  full)
    RUN_ARGS=(
      ...
      --wandb-group nabla-nte64-edge-norm-ablation-v1
      --wandb-run-name "${WANDB_DISPLAY_NAME}"
    )
    SCOPE_SLUG=full-e20
    ;;
  *)
    echo "Usage: $0 {validate|smoke|full} {postrms|split|degree} GPU0,GPU1" >&2
    exit 2
    ;;
esac
...
OUTPUT_ROOT=${PROJECT_ROOT}/outputs/nabladft-nte64-${ABLATION_SLUG}-2gpu-eb20-mb5-ga2-${SCOPE_SLUG}-${RUN_ID}
...
if [[ "${SCOPE}" == "smoke" && ${EXIT_CODE} -eq 0 ]]; then
  case "${OUTPUT_ROOT}" in
    "${PROJECT_ROOT}"/outputs/nabladft-nte64-edge-*-2gpu-eb20-mb5-ga2-full-size-smoke-e1-*) ;;
    *)
  ...
  rm -rf -- "${OUTPUT_ROOT}"
  echo "Smoke passed for ${ABLATION}; temporary artifacts removed."

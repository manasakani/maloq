...
# Native interleaved MALOQ backbone + corrected MUON-compatible MALOQ head.
...
GROUP_ROOT=${PROJECT_ROOT}/outputs/nabladft-maloq-muon-head-2gpu-eb20-mb5-ga2-${SCOPE_SLUG}-seed44-${RUN_ID}
MODEL_OUTPUT=${GROUP_ROOT}/maloq
LOG_FILE=${GROUP_ROOT}/logs/maloq.log
...
printf 'maloq\tesen-interleaved\tmaloq_muon\t%s\t5\t2\t2\t20\t%s\n' \
...
echo "MALOQ ${SCOPE}: ${status} (exit ${exit_code})"

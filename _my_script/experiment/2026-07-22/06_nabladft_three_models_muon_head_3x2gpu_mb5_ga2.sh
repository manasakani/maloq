...
# Run the three matched NablaDFT backbones with the Muon-compatible MALOQ head.
#
# GPU mapping:
#   MALOQ     -> 0,1
#   MALOQ-NTE -> 2,3
...
GROUP_ROOT=${PROJECT_ROOT}/outputs/nabladft-three-model-muon-head-parallel-3x2gpu-eb20-mb5-ga2-${SCOPE_SLUG}-seed44-${RUN_ID}
...
printf 'model\thead_type\tgpus\tmaster_port\tmicro_batch\taccumulation\teffective_batch\toutput\n' \
...
launch_model() {
  ...
  printf '%s\tmaloq_muon\t%s\t%s\t5\t2\t20\t%s\n' \
  ...
  MODEL_PIDS[${model}]=$!
  echo "Started ${model} with maloq_muon on GPUs ${gpus}: PID ${MODEL_PIDS[${model}]}"
  echo "  log: ${log_file}"
}
...
if [[ "${SCOPE}" == "smoke" && ${overall_status} -eq 0 ]]; then
  case "${GROUP_ROOT}" in
    "${PROJECT_ROOT}"/outputs/nabladft-three-model-muon-head-parallel-3x2gpu-eb20-mb5-ga2-full-size-smoke-e1-seed44-*) ;;
    *)

...
RUNNER=${SCRIPT_ROOT}/run_nabladft_v2_ofat.py
BASE_CONFIG=${SCRIPT_ROOT}/nabladft_v2_ofat_common.yaml
...
for required in "${PY}" "${MPIRUN}" "${RUNNER}" "${BASE_CONFIG}" "${NABLA_DB}" "${SCALE_SHIFT}"; do
  if [[ ! -r "${required}" ]]; then
...
if [[ ! -x "${PY}" || ! -x "${MPIRUN}" || ! -x "${RUNNER}" ]]; then
  echo "Python, mpirun, and the OFAT runner must be executable." >&2
  exit 1
fi
...
OUTPUT_ROOT=${PROJECT_ROOT}/outputs/${RUN_PREFIX}-${LANE}-2gpu-eb20-mb5-ga2-${SCOPE_SLUG}-${RUN_ID}
...
if [[ "${SCOPE}" == "smoke" && ${EXIT_CODE} -eq 0 ]]; then
  case "${OUTPUT_ROOT}" in
    "${PROJECT_ROOT}"/outputs/"${RUN_PREFIX}"-"${LANE}"-2gpu-eb20-mb5-ga2-full-size-smoke-e1-*) ;;
    *)

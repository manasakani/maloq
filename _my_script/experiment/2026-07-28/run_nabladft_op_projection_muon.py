#!/usr/bin/env python3
"""Train the matrix-free NTE-V2 operator projection with shape-routed Muon."""
...
DEFAULT_CONFIG = EXPERIMENT_ROOT / "nabladft_op_projection_muon.yaml"
...
for source_path in (PROJECT_ROOT, SOURCE_ROOT, EXPERIMENT_ROOT):
...
from op_projection_muon_lane import (  # noqa: E402
    OpProjectionMuonTrainingConfig as OpProjectionTrainingConfig,
...
def _database_metadata(database_path: Path) -> dict[str, Any]:
    ...
    if rows != EXPECTED_DATABASE_ROWS:
        raise ValueError(
            f"database row count drifted: {rows} != {EXPECTED_DATABASE_ROWS}"
        )
...
def _validate_preview(
    ...
    model = _build_model(
        ...
        orbital_basis=_normalize_orbital_basis(basis_sets.orbital_basis_def2_svp_nabla),
        orbital_template=cache["orbital_template"],
        ls_list=cache["ls_list"],
        device=torch.device("cpu"),
    )
    result = {
        "lane": config.model.model_variant,
        "scope": "validate",
...
def _validate_epoch(
    ...
    *,
    ...
    for batch_index, batch in enumerate(val_loader):
        ...
        loss, squared_error, target_squared, molecule_count = molecule_probe_statistics(
            predicted_action,
            target_action,
            molecule_ptr,
        )
    ...
    *,
    ...
    metrics = {
        ...
        f"{prefix}/ao_diagonal_mae_micro": (
            reduced["diagonal_absolute_error_sum"] / reduced["diagonal_entry_count"]
        ),
        ...
        f"{prefix}/coverage_fraction": (molecule_count / expected_global_molecules),
        f"{prefix}/evaluated_entries": entry_count,
        f"{prefix}/mean_ao_dimension": (reduced["ao_dimension_sum"] / molecule_count),
    }
    if split == "validation":
        metrics.update(
            {
...
def _run_training(
    ...
    *,
    ...
    try:
        ...
        optimizer, optimizer_provenance = build_muon_optimizer(
            ddp_model.module,
            config.optimization,
        )
        ...
        if rank == 0:
            resolved = {
                ...
                "consumed_train_rows": (config.dataset.num_train // world_size)
                * world_size,
            ...
            with (output_root / "optimizer_groups.json").open("w") as handle:
                json.dump(optimizer_provenance, handle, indent=2, sort_keys=True)
                handle.write("\n")
        ...
        for epoch in range(config.optimization.num_epochs):
            ...
            for batch_index, batch in enumerate(train_loader):
                ...
                sync_context = (
                    contextlib.nullcontext() if sync_gradients else ddp_model.no_sync()
                )
                with sync_context:
                ...
                if global_step == 0:
                    missing_gradients = _missing_gradient_parameters(ddp_model.module)
            ...
            if (
                ...
                exact_matrix_metrics.update(
                    _exact_matrix_metrics(
                        ...
                        train_loader,
                        split="train",
                        samples_per_rank=(config.matrix_metrics.train_samples_per_rank),
                        identity_column_chunk_size=(
                            config.matrix_metrics.identity_column_chunk_size
                        ),
                        expected_global_molecules=(
                            config.matrix_metrics.train_samples_per_rank * world_size
                        ),
                        device=device,
                    )
                )
            ...
            if wandb_run is not None:
                wandb_payload = {
                    ...
                    "validation/total_loss": validation["validation_probe_mse"],
                    **{key: value for key, value in validation.items() if "/" in key},
                    **exact_matrix_metrics,
                    **optimizer_learning_rates(optimizer),
                    "time/train_epoch_seconds": train_epoch_seconds,
                    "time/epoch_seconds": epoch_seconds,
                    "system/gpu_peak_memory_mb": (peak_cuda_memory_bytes / (1024.0**2)),
                }
                wandb_run.log(wandb_payload, step=global_step)
        ...
        final = {
            ...
            "muon_and_aux_adamw_state_verified": scope == "smoke",
        ...
        final.update(
            {key: value for key, value in last_epoch_payload.items() if "/" in key}
        )
...
def main() -> None:
    ...
    if scope == "validate":
        ...
        print(
            json.dumps(_validate_preview(config, config_path), indent=2, sort_keys=True)
        )
        return

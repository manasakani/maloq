...
RUNNER=${SCRIPT_ROOT}/run_nabladft_op_projection_muon.py
CONFIG=${SCRIPT_ROOT}/nabladft_op_projection_muon.yaml
NABLA_DB=/dataset/seongsu/shared-home/datasets/nablaDFT/hamiltonian_databases/train_2k.db
RUN_PREFIX=nabladft-ntev2-op-projection-full-val-matrix-metrics-v4-muon

usage() {
  echo "Usage: $0 {prepare|validate|smoke|full} [GPU0,GPU1]" >&2
}

for required in "${PY}" "${MPIRUN}" "${RUNNER}" "${CONFIG}" "${NABLA_DB}"; do
  if [[ ! -r "${required}" ]]; then
...
if [[ ! -x "${PY}" || ! -x "${MPIRUN}" || ! -x "${RUNNER}" ]]; then
  echo "Python, mpirun, and the dedicated runner must be executable." >&2
  exit 1
fi
...
if [[ "${SCOPE}" == "prepare" || "${SCOPE}" == "validate" ]]; then
  if [[ -n "${GPUS}" ]]; then
    echo "${SCOPE} does not accept GPU indices." >&2
    usage
    exit 2
  fi
  exec env \
...
OUTPUT_ROOT=${PROJECT_ROOT}/outputs/${RUN_PREFIX}-2gpu-eb20-mb5-ga2-${SCOPE_SLUG}-${RUN_ID}
...
if [[ "${SCOPE}" == "smoke" && ${EXIT_CODE} -eq 0 ]]; then
  case "${OUTPUT_ROOT}" in
    "${PROJECT_ROOT}"/outputs/"${RUN_PREFIX}"-2gpu-eb20-mb5-ga2-full-size-smoke-e1-*) ;;
    *)
      echo "Refusing to remove unexpected smoke output: ${OUTPUT_ROOT}" >&2
      exit 1
      ;;
  esac
  if [[ -r "${OUTPUT_ROOT}/run/final_metrics.json" ]]; then
    cat "${OUTPUT_ROOT}/run/final_metrics.json"
  fi
  rm -rf -- "${OUTPUT_ROOT}"

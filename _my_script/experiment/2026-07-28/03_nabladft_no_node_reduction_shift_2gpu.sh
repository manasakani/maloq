...
RUNNER=${SCRIPT_ROOT}/run_nabladft_no_node_reduction_shift.py
...
if [[ ! -x "${PY}" || ! -x "${MPIRUN}" || ! -x "${RUNNER}" ]]; then
  echo "Python, mpirun, and the ablation runner must be executable." >&2
  exit 1
fi
...
OUTPUT_ROOT=${PROJECT_ROOT}/outputs/${RUN_PREFIX}-${LANE}-no-node-reduction-2gpu-eb20-mb5-ga2-${SCOPE_SLUG}-${RUN_ID}
...
if [[ "${SCOPE}" == "smoke" && ${EXIT_CODE} -eq 0 ]]; then
  case "${OUTPUT_ROOT}" in
    "${PROJECT_ROOT}"/outputs/"${RUN_PREFIX}"-"${LANE}"-no-node-reduction-2gpu-eb20-mb5-ga2-full-size-smoke-e1-*) ;;
    *)

...
RUNNER=${SCRIPT_ROOT}/run_qhflow3_e3_esen_grid_scale_nabladft.py
CONFIG=${SCRIPT_ROOT}/qhflow3_e3_esen_grid_scale_nabladft.yaml
...
RUN_PREFIX=nabladft-v2-qhflow3-e3-10x11-muon-scale

usage() {
  echo "Usage: $0 {prepare|validate|smoke|full} [GPU0,GPU1]" >&2
}

for required in "${PY}" "${MPIRUN}" "${RUNNER}" "${CONFIG}" "${NABLA_DB}" "${SCALE_SHIFT}"; do
  if [[ ! -r "${required}" ]]; then

...
SCRIPT_ROOT=${PROJECT_ROOT}/_my_script/experiment/2026-07-28/native_raw
...
RUNNER=${SCRIPT_ROOT}/run_nabladft_native_raw.py
BASE_CONFIG=${SCRIPT_ROOT}/nabladft_native_raw_common.yaml
...
if [[ ! -x "${PY}" || ! -x "${MPIRUN}" || ! -x "${RUNNER}" ]]; then
  echo "Python, mpirun, and the native-RAW runner must be executable." >&2
  exit 1
fi
...
OUTPUT_ROOT=${PROJECT_ROOT}/outputs/${RUN_PREFIX}-${LANE}-native-raw-2gpu-eb20-mb5-ga2-${SCOPE_SLUG}-${RUN_ID}
...
if [[ "${SCOPE}" == "smoke" && ${EXIT_CODE} -eq 0 ]]; then
  case "${OUTPUT_ROOT}" in
    "${PROJECT_ROOT}"/outputs/"${RUN_PREFIX}"-"${LANE}"-native-raw-2gpu-eb20-mb5-ga2-full-size-smoke-e1-*) ;;
    *)

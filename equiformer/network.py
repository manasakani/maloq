...
from equiformer.transformer_block import NodeBlockV2, EdgeBlockV2
from equiformer.SO3 import SO3_Rotation, SO3_Embedding
...
class SO2Net(torch.nn.Module):
    ...
    def forward(self, batch):
        ...
        out = {
            "node_rankN": node_output,
            "edge_rankN": edge_output,
        }
        return out
...
class SO2Net_local(torch.nn.Module):
    ...
    def __init__(
        ...
        self.node_lin = Linear(irreps_in=irreps_in, irreps_out=irreps_out, biases=True)
        self.edge_lin = Linear(irreps_in=irreps_in, irreps_out=irreps_out, biases=True)
        # self.node_lin = e3nn_nn.FullyConnectedNet(irreps_in, [32, 64], irreps_out, act=[torch.relu, torch.relu])
        # self.edge_lin = e3nn_nn.FullyConnectedNet(irreps_in, [32, 64], irreps_out, act=[torch.relu, torch.relu])
    ...
    def forward(
        ...
        edge_embedding = self.blocks[-1](
                        node_embedding,                  # finally, update the edge embeddings with the final node and hidden embeddings 
                        atomic_numbers,
                        edge_distance_embedding,
                        edge_index,
                        hidden_embedding,
